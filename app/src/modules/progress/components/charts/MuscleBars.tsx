...
import { formatLoad, loadUnit } from './loadFormat';
...
export function MuscleBars({ data }: MuscleBarsProps) {
  ...
  const unit = loadUnit(max);

  return (
    <View className="gap-[0.6875rem]">
      {data.map((item, index) => {
        const colors = toneColors(cores, TONES[index % TONES.length]);
        const difference = item.current - item.previous;
        const label = labelOf(item.muscle);
