...
describe('useStudentRegistration', () => {
  it('começa na etapa de dados, sem serviço selecionado', () => {
    ...
    expect(result.current.step).toBe('data');
    expect(result.current.serviceTypes).toEqual([]);
  });

  it('alterna um serviço oferecido pelo especialista', () => {
    const { result } = renderHook(() =>
      useStudentRegistration({ offeredServices: ['personal_training', 'nutrition_consulting'] })
    );

    act(() => result.current.toggleService('personal_training'));
    expect(result.current.serviceTypes).toEqual(['personal_training']);

    act(() => result.current.toggleService('nutrition_consulting'));
  ...
  it('impede avançar sem nome, e-mail ou nenhum serviço selecionado', () => {
    ...
    expect(result.current.blockingReason).toBe('Preencha nome, e-mail e o tipo de acompanhamento.');
  ...
  it('conclui o convite e guarda o id do aluno criado', () => {
    ...
    act(() => result.current.completeInvite('aluno-9'));
