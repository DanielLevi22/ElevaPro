...
describe('refeições do dia', () => {
  ...
  it('dieta única mostra as refeições em qualquer dia', () => {
    ...
    for (let dia = 0; dia < 7; dia++) {
      expect(refeicoesDoDia(meals, 'unique', dia)).toHaveLength(2);
    }
  });
  ...
  it('dieta cíclica mostra só o dia escolhido', () => {
    ...
    expect(refeicoesDoDia(meals, 'cyclic', 1).map((m) => m.id)).toEqual(['seg']);
    expect(refeicoesDoDia(meals, 'cyclic', 2).map((m) => m.id)).toEqual(['ter']);
  });
  ...
  it('domingo é um dia como os outros na dieta cíclica', () => {
    ...
    expect(refeicoesDoDia(meals, 'cyclic', 0).map((m) => m.id)).toEqual(['dom']);
    expect(refeicoesDoDia(meals, 'cyclic', 1)).toEqual([]);
  });
  ...
  it('cíclica sem dia gravado não vira refeição de todo dia', () => {
    expect(refeicoesDoDia([refeicao()], 'cyclic', 3)).toEqual([]);
  });

  it('plano sem tipo conhecido continua filtrando por dia', () => {
    const meals = [refeicao({ id: 'qua', day_of_week: 3 })];

    expect(refeicoesDoDia(meals, null, 3).map((m) => m.id)).toEqual(['qua']);
    expect(refeicoesDoDia(meals, undefined, 4)).toEqual([]);
  });
});
