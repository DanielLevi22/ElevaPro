...
|-----|---------|--------|--------|
| [session-feedback-correction](PRDs/session-feedback-correction.md) | Direito de correcao do feedback de sessao (Art. 18, III) e fechamento do UPDATE/DELETE irrestrito da RLS | approved | `feature/session-feedback-correction` |
| [student-activity-feed](PRDs/student-activity-feed.md) | Aba Atividades: acompanhamento diario do aluno com feedback, treino, cardio e refeicoes | ✅ done | `feature/student-activity-feed` |
...
| 71 | ~~**Direito de correção (Art. 18, III) sem caminho**~~ — resolvido para o feedback de treino na migration `0036` (2026-08-28): o aluno edita `intensity` e `notes` da própria sessão e apaga só a observação, pelo histórico em `/student/session-history`. Junto veio o que ninguém tinha visto: a `sessions_own` da `0017` era `FOR ALL` e concedia **UPDATE de qualquer coluna e DELETE** ao aluno — controle que a seção 10 do LGPD_COMPLIANCE afirmava existir e o banco não tinha. Agora são políticas por comando, sem DELETE, com `GRANT UPDATE` de três colunas, tudo provado em `verify-rls.sql` com prova negativa. Anamnese e adesão à refeição já tinham caminho | ✅ | [PRD](PRDs/session-feedback-correction.md) |
| 72 | **`workoutStore.ts` e `workouts.service.ts` cresceram acima de 500 linhas no PR que os tocou** (753 e 519), contra a regra registrada na 16b. Não foram divididos junto com a migration de propósito: `workoutStore` precisa virar três fatias para caber, e fazer isso no mesmo PR que muda o schema misturaria regressão de refatoração com regressão de dado. A fatia natural é a de sessões — `saveWorkoutSession`, `saveCardioSession`, `fetchLastWorkoutSession`, `fetchWorkoutSessionDetails` —, que já tem 29 casos de teste | 🟡 Média | [PRD](PRDs/student-activity-feed.md) |
| 73 | **`FOR ALL` sem decisão registrada em três tabelas de dado sensível.** `anamnesis_own` e `body_scans_own` (`0017`) e `student_own_meal_logs` (`0013`) concedem UPDATE e DELETE ao aluno da mesma forma que a `sessions_own` concedia — e foi assim que o furo da `workout_sessions` nasceu. Em `body_scans` o DELETE **passou a ser usado de propósito** (Art. 18, VI) e está certo; nos outros dois ninguém decidiu, e o que existe é permissão sem desenho. Cada um precisa da mesma pergunta que a `0036` respondeu: o titular corrige a declaração, e o que é medida fica | 🟡 Média | [PRD](PRDs/session-feedback-correction.md) |
| 74 | **B1 — preview da Vercel atrás do SSO, e `SUPABASE_DB_URL` de preview intermitente.** `curl https://elevapro-preview.vercel.app/api/ai/body-scan` responde `302` para `vercel.com/sso-api`: o `Deployment Protection` protege o deployment inteiro, rotas de API incluídas, e é por isso que a IA do mobile não alcança o BFF. Separado: o job `migrate` falhou três vezes em 27/08 com `tenant/user postgres.lcwzijtunmlvvrqtproh not found` e passou em 28/08 — intermitente em migration é o pior tipo. **Nenhum dos dois é código:** são painel da Vercel e secret do GitHub. O lado do app já está pronto — o helper de BFF nomeia o host e distingue "não é JSON da aplicação" de "falhou" | 🔴 Alta | [PRD](PRDs/session-feedback-correction.md) |
| 75 | **Nenhuma trava LGPD nas features anteriores à regra.** A skill `/lgpd-check` passou a exigir teste de trava por parecer (2026-08-28), com prova negativa. As features já entregues não têm: `student_consents`, o gate de consentimento, a imutabilidade de `physical_assessments` e a política do bucket `assessments` são decisões jurídicas sem teste que quebre quando alguém as afrouxar. Aplicar retroativamente, começando pelas que a seção 10 marca como revisadas | 🟡 Média | [PRD](PRDs/session-feedback-correction.md) |
