...
| 44 | **Nenhum caminho grava `physical_assessments` corretamente** — mobile e web usam nomes de coluna que não existem, e os dois desligam a checagem (`const { data }` sem `error`; `as unknown as AssessmentInsert`). A leitura devolve "sem avaliação" em vez de erro. Bloqueia a régua do body scan (`ADR-010`) | 🔴 Crítica | [PRD](PRDs/physical-assessment-schema-drift.md) |
